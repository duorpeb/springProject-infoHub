...
### **1. API 의 사용**
- 이전 팀 프로젝트를 하며 API 를 한 번 사용해봤을 때, 느낀 점은 API 를 사용하기 전 API 가 어떤 형식으로 데이터를 뿌려주는지 알면 좋을 것 같다는 생각이 들었었습니다. 그래서 이런 저런 Tool 을 찾아보다가 Postman 을 이용해 네이버 검색 API 와 기상청 단기 예보 API 가 어떤 형식으로 데이터를 뿌려주는지 미리 파악하고 API 를 사용했고 이는 설계 단계에서 페이지의 구조를 설계할 때, 이전보다 디테일한 설계를 할 수 있게 해주었던 것 같습니다.

- 또한 설계 단계에서 메인 페이지의 뉴스 기사 10개 를 출력할 때, 
